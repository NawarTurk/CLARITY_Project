...
# ================= PATHS =================
test_data_path = "codebench_test.csv"
...
# ================= MODEL =================
MODEL_REGISTRY = {
    "qwen3-235b-instruct": ("Qwen/Qwen3-235B-A22B-Instruct-2507", "together"),
}
...
for llm_name, (model_id, provider) in MODEL_REGISTRY.items():
    ...
    out_path = os.path.join(prediction_path, f"{pred_col}", f"{pred_col}.csv")
    ...
    print(f"Running model: {llm_name}")
    ...
    for i, row in tqdm(df.iterrows(), total=len(df)):
        ...
        # ===== TOP-LEVEL LABEL ALREADY EXISTS =====
        ...
        # ---- Speaker/president (optional) ----
        president = row.get("president", None)
        speaker_line = ""
        if isinstance(president, str) and president.strip():
            speaker_line = f"Speaker: {president.strip()}\n"

        # ---- Build prompt ----
        user_msg = (
            f"Target question: {row.get('question','')}\n"
            f"{speaker_line}"
            f"Full interviewer turn: {row.get('interview_question','')}\n"
            f"Answer: {row.get('interview_answer','')}\n"
            f"Label:"
        )
