...
TUNE_STRATEGY = "lora"  # <-- LoRA strategy name for slug
...
def _guess_lora_target_modules(model) -> list[str]:
    """
    Pick attention projection module names that exist in this model.
    Works for BERT/RoBERTa/XLM-R and DeBERTa-v3 style models.
    """
    model_type = (getattr(getattr(model, "config", None), "model_type", "") or "").lower()

    if any(x in model_type for x in ["bert", "roberta", "xlm-roberta"]):
        return ["query", "value"]
    if "deberta" in model_type:
        return ["query_proj", "value_proj"]

    # Fallback: discover common projection names present in the model
    candidates = set()
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            leaf = name.split(".")[-1]
            if leaf in {"query", "value", "q_proj", "v_proj", "query_proj", "value_proj"}:
                candidates.add(leaf)
    return sorted(candidates) if candidates else ["query", "value"]


def _force_only_lora_trainable(model) -> None:
    """
    Guarantee: ONLY LoRA weights get gradients (freeze classifier/head too).
    """
...
def _plot_loss_curves(trainer: Trainer, run_name: str) -> None:
    ...
    # Force integer epoch ticks on the x-axis (no .5 values)
...
def train_model(
    ...
    *,
    ...
    # Freeze EVERYTHING first (including classifier head)
    for p in base_model.parameters():
        p.requires_grad = False

    targets = _guess_lora_target_modules(base_model)
    lora_cfg = LoraConfig(
        r=LORA_R,
    ...
    # Try with explicit targets first; if PEFT reports that no modules were
    # matched, fall back to its built-in defaults for this architecture.
    try:
        ...
        if "No modules were targeted for adaptation" not in str(e):
            raise
        print(
            "[Warn] LoRA target_modules did not match any layers; "
            "falling back to PEFT defaults for this backbone."
        )
        lora_cfg = LoraConfig(
            r=LORA_R,
            lora_alpha=LORA_ALPHA,
    ...
    # Extra safety: ensure only LoRA params are trainable
    _force_only_lora_trainable(model)
    ...
    trainer = WeightedTrainer(
        ...
        processing_class=tokenizer,   # mirrors your current usage
        compute_metrics=compute_metrics,
        callbacks=callbacks,
    )

    print("[Info] Starting LoRA training (base frozen, LoRA only)…")
    trainer.train()
    print("best ckpt:", trainer.state.best_model_checkpoint)
    print("best metric:", trainer.state.best_metric)
    print("exists?", os.path.exists(trainer.state.best_model_checkpoint or ""))
    ...
    # With load_best_model_at_end=True, trainer.model is already the best.
    os.makedirs(final_model_dir, exist_ok=True)
    trainer.save_model(final_model_dir)     # for PEFT: saves adapter weights + config
    tokenizer.save_pretrained(final_model_dir)
    print(f"[Info] Saved LoRA adapter & tokenizer to {final_model_dir}")
...
# -----------------------------------------------------------------------------
# 4) Pipeline: load CSVs, train on ALL train, validate on TEST
# -----------------------------------------------------------------------------
def main():
    ...
    for resolved_model in model_names:
        print(f"\n[Run] Training {resolved_model} with LoRA-only, mode={args.param_mode}")
