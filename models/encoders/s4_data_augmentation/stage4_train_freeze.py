...
def main():
    ...
    parser.add_argument(
        ...
        default="atomic",
        help="Input mode: atomic uses question; enriched uses question+interview_question.",
    )
    parser.add_argument(
        "--model_name",
        required=True,
    ...
    for resolved_model in model_names:
        for unfreeze_ratio in unfreeze_ratios:
            print(
                ...
                f"unfreeze_ratio={unfreeze_ratio:.2f}, dataset={args.dataset}, "
                f"input_mode={args.input_mode}), mode={args.param_mode}"
            )

            train_path = TRAIN_CSV_PATH
            if args.dataset == "augmented":
                train_path = AUGMENTED_TRAIN_CSV_PATH
