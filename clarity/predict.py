...
def _generate_prediction(prompt: str, generator, *, max_new_tokens: int, do_sample: bool) -> str:
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a political expert that must classify with exactly one label: "
    ...
    generation_output = generator(formatted_prompt, **generator_kwargs)[0]["generated_text"]

    if not uses_chat_template and generation_output.startswith(formatted_prompt):
        generation_output = generation_output[len(formatted_prompt) :]
